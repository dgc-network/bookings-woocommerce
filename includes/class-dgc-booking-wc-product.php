...
class WC_Product_dgc_booking extends WC_Product {
	
	private $booked_price = false;

	public function __construct( $product ) {
		$this->product_type = 'dgc_booking';
	...
	public function get_interval_type(){
		return get_post_meta( $this->id, "_dgc_book_interval_type", 1 );
	}

	public function get_interval_period(){
		return get_post_meta( $this->id, "_dgc_book_interval_period", 1 );
	}

	public function get_interval(){
		return get_post_meta( $this->id, "_dgc_book_interval", 1 );
	}
	...
	public function dgc_set_booked_price( $id, $price ) {
	...
	public function get_price($context = 'view'){
		...
		$customer_value = isset(WC()->customer) ? WC()->customer->get_meta('dgc_booking_details') : '';
		if( !empty($customer_value[ $this->id ]) && $customer_value[ $this->id ]['book_to'] ){
			...
			$prod_interval 				= get_post_meta( $this->id, '_dgc_book_interval', 1 );
			
			//Default interval 1
			if( empty($prod_interval) )
				$prod_interval 		= 1;
		
			$prod_price_per_interval	= get_post_meta( $this->id, '_dgc_book_price', 1 );
			$prod_price_interval_type	= get_post_meta( $this->id, '_dgc_book_interval_period', 1 );
			...
			$price = get_post_meta( $this->id, '_dgc_book_price', 1 );
		}
		$final_price = apply_filters('dgc_booking_cost',$price,$this->id); 
	...
	function is_purchasable() {
		...
		if ( true === $purchasable && false === WC_Product_dgc_booking::is_purchasable( $purchasable, $this ) ) {
			$purchasable = false;
		}
