...
<div id='booking_options' class='panel woocommerce_options_panel'><?php
	$interval_period 	= get_post_meta( $post->ID, '_dgc_book_interval_period', 1);
	$interval_type 		= get_post_meta( $post->ID, '_dgc_book_interval_type', 1);
	$interval 			= get_post_meta( $post->ID, '_dgc_book_interval', 1);
	$price 				= get_post_meta( $post->ID, '_dgc_book_price', 1);
	$opening_time		= get_post_meta( $post->ID, '_dgc_book_working_hour_start', 1);
	$closing_tme		= get_post_meta( $post->ID, '_dgc_book_working_hour_end', 1);
	?>
	<p class="form-field" style="line-height: 15px;">
		<label for="_dgc_book_price" ><?php _e('Booking Period','bookings-and-appointments-for-woocommerce')?></label>
		<select id="_dgc_book_interval_type" name="_dgc_book_interval_type" class="input-item">
		...
		<input type="number" onKeyPress="if(this.value.length==3) return false;" class="short input-item" style="width:50px;margin-left: 10px;" name="_dgc_book_interval" id="_dgc_book_interval" value="<?php echo $interval;?>" placeholder="1">
		<select id="_dgc_book_interval_period" name="_dgc_book_interval_period" class="select short input-item" style="width:85px;margin-left: 10px;" >
	...
	<p class="form-field" >
		<label for="_dgc_book_price" ><?php _e('Price','bookings-and-appointments-for-woocommerce')?></label>
		<input type="text" class="short input-item" style="width:70px" name="_dgc_book_price" id="_dgc_book_price" value="<?php echo $price;?>" placeholder=""><br>
		<i><?php
	...
	woocommerce_wp_text_input( array(
		'id'			=> '_dgc_book_working_hour_start',
