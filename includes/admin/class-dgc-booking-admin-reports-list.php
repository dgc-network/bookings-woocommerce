...
class dgc_booking_all_list extends WP_List_Table {
	protected $max_items;
	...
	*/
	...
	private function is_confrimed_all_bookings_of_order($order){
		...
		foreach ($items as $order_item_id => $line_item) {
			...
			$required_confirmation 	= get_post_meta( $_product->get_id(), "_dgc_book_required_confirmation", 1 );
	...
	*/
	public function get_items( $current_page, $per_page ) {
		...
		$filters = array(
			'dgc_booking_status' 		=> isset( $_GET['dgc_booking_status'] ) ? $_GET['dgc_booking_status'] : '',
			'dgc_filter_product_ids' 	=> isset( $_GET['dgc_filter_product_ids']) ? $_GET['dgc_filter_product_ids'] : '',
			'dgc_filter_from' 			=> isset( $_GET['dgc_filter_from']) ? $_GET['dgc_filter_from'] : '',
			'dgc_filter_to' 				=> isset( $_GET['dgc_filter_to']) ? $_GET['dgc_filter_to'] : '',
		);
		$this->max_items 	= $this->dgc_get_bookings_count( $filters, $current_page, $per_page );
		$this->items 		= $this->dgc_get_bookings_for_current_page( $filters, $current_page, $per_page );
	...
	*
	*/
	private function dgc_get_bookings_count( $filters, $current_page, $per_page ){
		...
		AND t.slug = 'dgc_booking'";

		if( !empty($filters['dgc_booking_status']) ){
			$query .= "AND imeta.BookingStatus = '".$filters['dgc_booking_status']."'";
		}
		if( !empty($filters['dgc_filter_product_ids']) ){
			$query .= "AND imeta.ProductId = '".$filters['dgc_filter_product_ids']."'";
		}
		if( !empty($filters['dgc_filter_from']) ){
			$query .= " AND (DATE(imeta.BookFrom) >= '".$filters['dgc_filter_from']."'";
			$query .= " OR DATE(imeta.BookedFrom) >= '".$filters['dgc_filter_from']."')";
		}elseif( !empty($filters['dgc_filter_to']) ){
			$query .= " AND( DATE(imeta.BookFrom) <= '".$filters['dgc_filter_to']."'";
			$query .= " OR DATE(imeta.BookedFrom) <= '".$filters['dgc_filter_to']."')";
		}
	...
	private function dgc_get_bookings_for_current_page( $filters, $current_page, $per_page ){
	...
	*/
	protected function display_tablenav( $which ) {
		...
		include( 'views/html-dgc-booking-admin-reports-list-filters.php' );
	}
